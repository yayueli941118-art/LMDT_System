...
render_page_banner("💰", "工资决定与收入差距", "第六章 工资与收入分配", "orange")
...
with tab1:
    ...
    with col_ctrl:
        ...
        st.markdown("<div class='card-header'>🏭 岗位特征 → 工资补偿</div>", unsafe_allow_html=True)
    ...
    with col_graph:
        ...
        fig1a.add_trace(go.Scatter(
            ...
            line=dict(color=COLOR["warning"], width=5),
            hovertemplate='风险=%{x:.2f}‰<br>补偿后工资=%{y:.1f}k<br>溢价=%{customdata:+.1f}k<extra></extra>',
        ...
        fig1a.add_trace(go.Scatter(
            ...
            marker=dict(size=22, color=COLOR["warning"], symbol='star', line=dict(width=3, color='white')),
    ...
    st.markdown(f"""
    <div style="background: #fffbeb; border-left: 3px solid {COLOR['warning']}; padding: 16px; border-radius: 4px; margin: 12px 0;">
...
with tab2:
    ...
    with col_ctrl2:
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        st.markdown("<div class='card-header'>⚡ 效率工资 & 最低工资</div>", unsafe_allow_html=True)
    ...
    eff_wage = mpl_eff
    eff_wage_min = no_shirk_condition + 2
    ...
    with col_graph2:
        ...
        demand_wage = mpl_eff * (1 - 0.005 * (L_range - 10))
        ...
        fig2a.add_hline(
            y=eff_wage_min, line_dash="dash", line_color=COLOR["warning"], line_width=2,
            annotation_text=f"效率工资 ≈{eff_wage_min:.1f}k (NSC)",
            annotation_font=dict(color=COLOR["warning"])
        )
        ...
        if min_wage > 0:
            fig2a.add_hline(
                y=min_wage, line_dash="dash", line_color=COLOR["danger"], line_width=2,
                annotation_text=f"最低工资={min_wage:.1f}k",
                annotation_font=dict(color=COLOR["danger"])
            )
            ...
            if L_min < market_L:
                ...
                fig2a.add_annotation(
                    ...
                    showarrow=False, font=dict(size=12, color=COLOR["danger"]),
        ...
        fig2a.update_layout(
            ...
            xaxis=dict(gridcolor="rgba(0, 0, 0, 0.08)"),
            yaxis=dict(range=[0, max(mpl_eff * 1.5, min_wage * 1.5 + 2)], gridcolor="rgba(0, 0, 0, 0.08)"),
            template="plotly_white",
            height=420, margin=dict(l=40, r=20, t=20, b=40),
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
            hovermode='closest'
    ...
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("<div class='card-header'>🔬 不偷懒条件 (NSC) 曲线</div>", unsafe_allow_html=True)
    ...
    with col_nsc2:
        ...
        fig2b.add_trace(go.Scatter(
            ...
            line=dict(color=COLOR["warning"], width=4),
        ...
        fig2b.add_trace(go.Scatter(
            ...
            marker=dict(size=16, color=COLOR["warning"], symbol='star', line=dict(width=2, color='white')),
...
# ==========================================
# TAB 3: Income Inequality
# ==========================================
with tab3:
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("<div class='card-header'>📊 收入不平等——洛伦兹曲线与基尼系数</div>", unsafe_allow_html=True)
    ...
    with col_lc2:
        ...
        fig3.add_trace(go.Scatter(
            ...
            line=dict(color=COLOR["success"], width=1.5, dash='dash'),
        ...
        fig3.add_trace(go.Scatter(
            ...
            hovertemplate='人口累计=%{x:.0f}%<br>收入累计=%{y:.0%}<br>Gini={customdata:.3f}<extra></extra>',
            customdata=[gini] * 200
        ))
        ...
        fig3.add_annotation(
            ...
            showarrow=False, font=dict(size=16, color=COLOR["warning"]),
        ...
        fig3.update_layout(
            ...
            yaxis=dict(range=[0, 100], gridcolor="rgba(0, 0, 0, 0.08)"),
            template="plotly_white",
    ...
    with col_g2:
        ...
        fig3b.add_trace(go.Scatter(
            ...
            line=dict(color=COLOR["danger"], width=3), marker=dict(size=8)
        ))
        fig3b.add_trace(go.Scatter(
            x=years, y=us_sp, mode='lines+markers', name='🇺🇸 美国',
            line=dict(color=COLOR["primary"], width=3), marker=dict(size=8)
        ))
        fig3b.add_hline(y=skill_premium, line_dash="dash", line_color=COLOR["warning"],
                        annotation_text=f"你的设定={skill_premium:.1f}×")
        
        fig3b.update_layout(
            xaxis_title="年份", yaxis_title="大学溢价 (倍数)",
            xaxis=dict(gridcolor="rgba(0, 0, 0, 0.08)"),
            yaxis=dict(range=[0.8, 2.5], gridcolor="rgba(0, 0, 0, 0.08)"),
            template="plotly_white",
...
# ==========================================
st.markdown("<div class='card'>", unsafe_allow_html=True)
st.markdown("<div class='card-header'>🏛️ 深度思考：工资的本质与共同富裕</div>", unsafe_allow_html=True)
